...
class LunaLib:
    ...
    def download_blockchain_with_progress(self, force_download=False) -> bool:
        """Download blockchain with progress tracking"""
        if not self.check_network_connection():
            self._update_sync_progress(0, "Network error - cannot connect")
            return False
        ...
        try:
            ...
            if current_height is None:
                self._update_sync_progress(0, "API error - cannot get blockchain height")
                return False
            
            if current_height == 0:
                self._update_sync_progress(0, "No blocks available")
                return False
            
            # Determine what we need to download
            cached_height = self.blockchain_cache.get_highest_cached_height()
            start_height = 0 if force_download else cached_height + 1
            
            if start_height > current_height:
                self._update_sync_progress(100, "Up to date")
                return True
            
            total_blocks = current_height - start_height + 1
            self._update_sync_progress(0, f"Downloading {start_height} to {current_height}")
            ...
            for batch_start in range(start_height, current_height + 1, batch_size):
                ...
                self._update_sync_progress(progress, f"Downloading blocks {batch_start}-{batch_end}")
                
                # Get blocks
                blocks = self._get_blocks_range(batch_start, batch_end)
                if not blocks:
                    self._update_sync_progress(0, "Download failed - no blocks received")
                    return False
                ...
                for block in blocks:
                    ...
                    self.blockchain_cache.save_block(height, block_hash, block)
                
                # Small delay to be nice to the server
                time.sleep(0.1)
            
            self._update_sync_progress(100, "Download complete")
            return True
            
        except Exception as e:
            print(f"Download error: {e}")
            self._update_sync_progress(0, f"Error: {str(e)}")
            return False
